...
> [!NOTE]
> Note o "ponto" no final do comando!
...
Inclua as seguintes linhas no arquivo `Carros/MTCars/settings.py` para permitir acesso de qualquer lugar ao site e evitar erros relacionados a *CORS*:
...
### Registrando a sua aplicação MTCars

> [!TIP]
> Se necessário, interrompa o servidor Django digitando `Ctrl+C` no terminal.

Registre a sua aplicação no arquivo `Carros/MTCars/settings.py`:
...
No diretório `Carros/MTCars/`, crie o arquivo `models.py` e inclua a seguinte classe nesse arquivo para representar a nossa base de dados de carros:
...
Copie os arquivos [`MTCars.sqlite3`](mtcars.sqlite3) e [`CopiaBaseDados.py`](CopiaBaseDados.py) para o diretório `Carros/`, o mesmo onde está o arquivo `db.sqlite3`.
...
```bash
python manage.py makemigrations MTCars
python manage.py migrate
```
...
Para poder usar a interface administrativa para examinar o banco de dados, registre o como como adminitrável editando o arquivo `MTCars.admin.py`:
...
> [!CAUTION]
> A variável `STATIC_URL` já deve existir no módulo, verifique antes de criar.

```python
STATIC_URL = "/static/" # inclua a / antes de static
```

Adicione um link para o arquivo de estilo no cabeçalho do template:
...
No diretório `Carros/MTCars`, crie o diretório `static/css`.
Crie o arquivo `estilo.css` e divirta-se com ele. 
...
Transforme o nome do carro em um link. Antes você tinha esse código no arquivo `templates/home.html`:
...
Edite o arquivo `Carros/MTCars/views.py` e inclua a seguinte `view`:
...
Crie o arquivo `Carros/MTCars/templates/detalhes.html` com o seguinte conteúdo:
...
<body>
    ...
    <a href="{% url 'home' %}">Voltar à lista de carros</a>
</body>
</html>
```

Registre a nova rota no arquivo `Carros/MTCars/urls.py`.
